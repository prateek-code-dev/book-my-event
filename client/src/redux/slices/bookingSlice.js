...
export const getAllBookingsFunction = createAsyncThunk(
    "getAllBookings",
    async (postData, thunkAPI) => {
        try {
            const result = await getAllBookingsAdminRequest(postData);
...
export const bookingSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            // Get all bookings (getAllBookingsFunction)
            .addCase(getAllBookingsFunction.pending, (state, action) => {
                state.loading = true;
            })
            .addCase(getAllBookingsFunction.fulfilled, (state, action) => {
                (state.loading = false), (state.allBookings = action.payload);
            })
            .addCase(getAllBookingsFunction.rejected, (state, action) => {
                state.loading = false;
                state.error = action.payload;
            });
    },
