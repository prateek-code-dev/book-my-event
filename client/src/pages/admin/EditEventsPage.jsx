...
const EditEventsPage = () => {
  ...
  const handleGuestRemove = (item, index) => {
    ...
    const updatedGuests = editEventData?.eventGuests?.filter((e) => e !== item);
  ...
  const handleAddGuest = () => {
    const updatedGuestList = [...[editEventData.eventGuests], addSingleGuest];
    setEditEventData({
      ...editEventData,
      ...
      <div className="flex flex-col gap-4 p-2 w-full">
            ...
            {editEventData &&
              ...
              editEventData?.eventGuests?.map((item, index) => (
                ...
                >
                  ...
                  <IoIosCloseCircle
                    size="20"
                    onClick={() => handleGuestRemove(item, index)}
                  />
                </div>
              ))}
          </div>
        ...
        <div className="flex items-center gap-4 w-full">
          ...
          <div className="border-2 w-full bg-gray-50">
            <DateSelector
              required
              placeHolderMessage={editEventData?.eventDate?.split("T")[0] || ""}
              placeholder={""}
              name="eventDate"
              type="date"
              onChange={handleChange}
          ...
          editEventData?.eventTickets?.length > 0 && (
            <div className="flex w-full">
              ...
              <div className="w-full">
                {editEventData?.eventTickets?.map((item, index) => (
                  <div key={index} className="w-full flex gap-2 items-center">
                    <Input value={item?.name} disabled />
                    <Input value={`Rs. ${item?.price}`} disabled />
                    <Input value={`${item?.limit} left`} disabled />
                    <IoIosCloseCircle
                      size="64"
                      onClick={() => handleRemoveTicketType(item, index)}
                    />
                  </div>
                ))}
              </div>
