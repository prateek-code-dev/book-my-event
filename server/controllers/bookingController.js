...
export const createBookingController = async (req, res, next) => {
  try {
    const { event, ticketType, ticketCount, totalAmount, paymentId } = req.body;

    if (!event || !ticketType || !ticketCount || !totalAmount || !paymentId) {
    ...
    if (!result) {
      return next(handleError(400, `Error! creating booking, Try Later!`));
    }
...
export const cancelBookingController = async (req, res, next) => {
  try {
    const { bookingId, eventId, ticketType, ticketCount } = req.body;

    if (!bookingId || !eventId || !ticketType || !ticketCount) {
      return next(handleError(400, `All fields are required!`));
    }
    ...
    if (!eventResult) {
      return next(handleError(400, `Error in updating events!`));
    }
